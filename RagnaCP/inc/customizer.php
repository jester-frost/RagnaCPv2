 ...
 */
function ragna_theme_customize_register( $wp_customize ) {
	$color_scheme = ragna_theme_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'ragna_theme_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'ragna_theme' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => ragna_theme_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'ragna_theme' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ragna_theme' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'ragna_theme' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ragna_theme' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'ragna_theme' );
}
add_action( 'customize_register', 'ragna_theme_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'ragna_theme_color_schemes'}.
 *
 ...
 */
function ragna_theme_get_color_schemes() {
	return apply_filters( 'ragna_theme_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'ragna_theme' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'ragna_theme_get_color_scheme' ) ) :
/**
 ...
 */
function ragna_theme_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = ragna_theme_get_color_schemes();
...
endif; // ragna_theme_get_color_scheme

if ( ! function_exists( 'ragna_theme_get_color_scheme_choices' ) ) :
/**
 ...
 */
function ragna_theme_get_color_scheme_choices() {
	$color_schemes                = ragna_theme_get_color_schemes();
...
endif; // ragna_theme_get_color_scheme_choices

if ( ! function_exists( 'ragna_theme_sanitize_color_scheme' ) ) :
/**
 ...
 */
function ragna_theme_sanitize_color_scheme( $value ) {
	$color_schemes = ragna_theme_get_color_scheme_choices();
...
endif; // ragna_theme_sanitize_color_scheme
 ...
 */
function ragna_theme_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = ragna_theme_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = ragna_theme_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = ragna_theme_get_color_scheme_css( $colors );

	wp_add_inline_style( 'ragna_theme-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'ragna_theme_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function ragna_theme_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', ragna_theme_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'ragna_theme_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function ragna_theme_customize_preview_js() {
	wp_enqueue_script( 'ragna_theme-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'ragna_theme_customize_preview_js' );
 ...
 */
function ragna_theme_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function ragna_theme_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-ragna_theme-color-scheme">
		<?php echo ragna_theme_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'ragna_theme_color_scheme_css_template' );
